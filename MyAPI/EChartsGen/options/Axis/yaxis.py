...
from MyAPI.EChartsGen.options.Axis.axisType import AxisType
...
class yAxis(EchartBaseObject):
    #TODO Data 可以是list
    def __init__(self, data = None, gridIndex= 0,
                             position= 'left',
                             type= AxisType('value'),
    ...
    def make_json(self):
        json = "yAxis:{"
        json += "{gridIndex}".format(gridIndex=self.gridIndex) if self.gridIndex else ""
