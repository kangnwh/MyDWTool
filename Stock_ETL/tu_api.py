...
def init_hist_data(engine):
	LOG_FILE = (os.getcwd()+os.sep+"logs"+os.sep+"init_load_%s.log") % (dt.datetime.now().strftime('%Y-%m-%d'))
	handler = lg.handlers.RotatingFileHandler(LOG_FILE, maxBytes=1024 * 1024, backupCount=5)  # 实例化handler
	fmt = '%(asctime)s - %(filename)s:%(lineno)s - %(name)s - %(message)s'
	formatter = lg.Formatter(fmt)  # 实例化formatter
	handler.setFormatter(formatter)  # 为handler添加formatter

	logger = lg.getLogger('init_load')  # 获取名为tst的logger
	logger.addHandler(handler)  # 为logger添加handler
	logger.setLevel(lg.DEBUG)

	logger.info('init load begin')
	logger.info("##########################################")

	# 获得上市公司的基本数据
	code_list = ts.get_stock_basics()
	code_list.to_sql('comp_basic', engine, schema='stock',index_label='seq_in_code', if_exists='fail')


	# 获得所有股票历史信息
	for code in code_list.index:
		logger.info(("Load data of code : %s begins") % (code))
		one_stock = ts.get_hist_data(code)

		try:
			one_stock['date'] = one_stock.index
			logger.info(("Code : %s has %d rows need to be inserted") % (code, one_stock.index.size))
		except Exception as e:
			logger.error(("Code : %s has something wrong ") % (code))
			continue

		one_stock = one_stock.reset_index(drop=True)  # used for 3 monthe analysis
		one_stock.insert(0, 'code', code, allow_duplicates=True)
		one_stock.to_sql('stock_hist', engine, schema='stock', if_exists='fail')
		logger.info(("Load data of code : %s successfully.") % (code))
		logger.info("##########################################")


def load_daily_data(engine, begin_code="000000"):
	...
	logger = lg.getLogger('daily_load')  # 获取名为tst的logger
	logger.addHandler(handler)  # 为logger添加handler
	logger.setLevel(lg.DEBUG)
	...
	# 获得上市公司的基本数据
	code_list = load_comp_basic(engine)
	...
	for code in code_list.index:
		...
		one_stock = one_stock.reset_index(drop=True)  # used for 3 monthe analysis
		one_stock.insert(0, 'code', code, allow_duplicates=True)
		one_stock.to_sql('stock_hist', engine,index_label='seq_in_code', schema='stock', if_exists='append')
		logger.info(("Code : %s updated successfully ") % (code))
		logger.info("##########################################")


def load_comp_basic(engine):
	LOG_FILE = (os.getcwd()+os.sep+"logs"+os.sep+"load_comp_basic_%s.log") % (dt.datetime.now().strftime('%Y-%m-%d'))
	handler = lg.handlers.RotatingFileHandler(LOG_FILE, maxBytes=1024 * 1024, backupCount=5)  # 实例化handler
	fmt = '%(asctime)s - %(filename)s:%(lineno)s - %(name)s - %(message)s'
	formatter = lg.Formatter(fmt)  # 实例化formatter
	handler.setFormatter(formatter)  # 为handler添加formatter
	...
	try:
		...
		engine.execute(str("truncate table stock.comp_basic"))
